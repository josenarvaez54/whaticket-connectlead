...
const ContactListItems = () => {
  ...
  const [statusParam, setStatus] = useState(initialSettings);
  ...
  useEffect(() => {
  console.log('useEffect esse :', );
    setLoading(true);
    const delayDebounceFn = setTimeout(() => {
      const fetchContacts = async () => {
        try {
        console.log(' statusParam:', statusParam);
          var isWhatsappValid = statusParam.isWhatsappValid
          if(isWhatsappValid === 0) isWhatsappValid = null;

          const { data } = await api.get(`contact-list-items`, {
            params: { isWhatsappValid, searchParam, pageNumber, contactListId },
          });
  ...
  }, [statusParam, searchParam, pageNumber, contactListId]);
  ...
  const handleOnChangeStatus = (e) => {
    ...
    setStatus((prev) => ({ ...prev, ...changedProp }));
  };
              ...
              <FormControl
                ...
                >
                  ...
                  <Select
                    ...
                    label="Status"
                    value={statusParam.isWhatsappValid}
                    onChange={(e) => handleOnChangeStatus(e)}
                  >
                    <MenuItem value={0}>Todos</MenuItem>
                    <MenuItem value={true}>Válido</MenuItem>
                    <MenuItem value={false}>Inválido</MenuItem>
            ...
            <>
              {contacts.map((contact) => (
                <TableRow key={contact.id}>
                  ...
                  <TableCell>{contact.isWhatsappValid ? 'Válido':'Inválido'}</TableCell>
